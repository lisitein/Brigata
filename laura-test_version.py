...
class IdentifiableEntity:
    ...
    def __eq__(self, other) -> bool:
        return type(self) is type(other) and getattr(other, "id", None) == self.id #added

    def __hash__(self) -> int:
        return hash((type(self), self.id)) #added
...
class Journal(IdentifiableEntity):
    ...
    def __eq__(self, other) -> bool:      
        ...
        # id：compared by set()
        if set(self.identifiers) != set(getattr(other, "identifiers", [])):
            return False #added

        # hasCategory / hasArea： string, but also Category/Area object
        def norm_ids(xs):
            out = []
            for x in xs or []:
                out.append(x.id if hasattr(x, "id") else x)
            return out
        ...
        return (
            ...
            and self.seal == other.seal
            and self.license == other.license
            and self.apc == other.apc
            and norm_ids(self.hasCategory) == norm_ids(other.hasCategory)
            and norm_ids(self.hasArea) == norm_ids(other.hasArea)
            ) #added

    def __hash__(self) -> int:
        return hash((type(self), tuple(sorted(self.identifiers)))) #added
    ...
    def getEntityById(self, id: str) -> Optional[Union[Journal, Category, Area]]:
        ...
        for h in self.journalHandlers:
            ...
            if not df.empty:
                ...
                return self._makeJournals(df)[0] #uncommmented by yang
        ...
        for h in self.categoryHandlers:
            ...
            if "category_id" in df.columns:
                if "id" in df.columns and "quartile" in df.columns: #added by Yang
                    row = df.iloc[0]
                    return Category(row["category_id"], row.get("quartile"))
            
            if "id" in df.columns and "quartile" in df.columns:
                row = df.iloc[0]
                return Category(row["id"], row.get("quartile")) # added by Yang
